...
/**
 * Unit tests for the Groq provider. `global.fetch` is replaced with a
 * controllable mock for every test — no real network call is ever made.
 * Mirrors `openrouter.provider.spec.ts`: the same timeout, abort, and
 * error-normalisation discipline is verified here, since Groq's API is
 * the same OpenAI-compatible chat-completions shape.
 */
describe('GroqProvider', () => {
  ...
  it('passes an explicit request.model override through to the request body', async () => {
    ...
    const provider = new GroqProvider(makeConfig());
    await provider.invoke(makeRequest({ model: 'llama-3.1-8b-instant' }));
  ...
  it('throws a CONFIGURATION AiProviderError without calling fetch when no API key is configured', async () => {
    ...
    (config.groq as { apiKey: string | undefined }).apiKey = undefined;
  ...
  it('throws a normalised INVALID_RESPONSE AiProviderError when the response has no choices', async () => {
    globalThis.fetch = jest
      .fn()
      .mockResolvedValue(
        jsonResponse(200, { id: 'chatcmpl-1', model: 'llama-3.3-70b-versatile', choices: [] }),
      ) as unknown as typeof globalThis.fetch;
