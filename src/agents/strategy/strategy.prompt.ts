 ...
 * for the Strategy Agent's instructions. This module never duplicates the
 ...
 * template's nine named placeholders from the request payload.
 *
 * Untrusted content handling (GDE-004 §6.7, §13.6): every rendered value —
 * trusted and untrusted alike — has the `<<<`/`>>>` delimiter sequences
 * neutralised before insertion, so no field (including `marketContext`, the
 * one UNTRUSTED block) can terminate a block early and inject sibling
 * content. `marketContext` size bounds (<=30 observations, <=500 code points
 * each) are enforced structurally by the input JSON Schema before this
...
@Injectable()
export class StrategyPromptService implements OnModuleInit {
  ...
  private loadAndParse(): ParsedPrompt {
    ...
    if (fenced.length !== 2 || systemPrompt === undefined || userTemplate === undefined) {
      throw new Error(
        `strategy-agent: expected exactly 2 fenced text blocks in system-prompt.md, found ${fenced.length}`,
      );
    }
    ...
    for (const match of userTemplate.matchAll(BLOCK_PATTERN)) {
      ...
      if (!isUserBlockKey(key)) {
        throw new Error(`strategy-agent: unknown template variable {{${key}}} in system-prompt.md`);
      }
    ...
    if (userTemplateBlocks.length !== USER_BLOCK_KEYS.length) {
      throw new Error(
        `strategy-agent: expected ${USER_BLOCK_KEYS.length} user-layer blocks in system-prompt.md, found ${userTemplateBlocks.length}`,
      );
    }
