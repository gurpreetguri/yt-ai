 ...
 * truth for the Script Writer Agent's instructions. This module never
 ...
 * fills the template's three named placeholders from the request payload.
 *
 * Structurally identical mechanism to `story-architect.prompt.ts` (same
 ...
 * insertion, applied to the ENTIRE `storyArchitecture` and
 * `verificationPackage` blocks — not a designated subset — since any nested
 * free-text field (a beat's purpose, a claim's text, a limitation) could
 * carry adversarial text originating from upstream research, even though
 * both blocks are themselves provenance-TRUSTED, already-validated platform
 * artifacts.
 */

export const SCRIPT_WRITER_PROMPT_ID = 'prm_script_writer_agent';

const PROMPT_RELATIVE_SEGMENTS = ['agents', 'agent-05-script-writer', 'system-prompt.md'] as const;
 ...
 * ever runs from:
 *  - the TypeScript source tree (`src/agents/script-writer/`, three levels
 *    below the repo root) during development and tests;
 *  - the compiled `dist/src/agents/script-writer/` output, three levels
 *    below `dist/`, where `npm run build` (`scripts/copy-prompt-asset.js`)
 *    copies the approved file verbatim to
 *    `dist/agents/agent-05-script-writer/` — the same relative depth.
 *
...
function resolvePromptFile(): string {
  ...
  throw new Error(
    `script-writer-agent: could not locate the approved prompt asset (system-prompt.md). Checked: ${candidates.join(', ')}. ` +
      'Confirm the production build ran "npm run build" (which copies this asset via scripts/copy-prompt-asset.js).',
  );
}

const USER_BLOCK_KEYS = ['storyArchitecture', 'verificationPackage', 'language'] as const;
...
@Injectable()
export class ScriptWriterPromptService implements OnModuleInit {
  ...
  private loadAndParse(): ParsedPrompt {
    ...
    if (fenced.length !== 2 || systemPrompt === undefined || userTemplate === undefined) {
      throw new Error(
        `script-writer-agent: expected exactly 2 fenced text blocks in system-prompt.md, found ${fenced.length}`,
      );
    }
    ...
    for (const match of userTemplate.matchAll(BLOCK_PATTERN)) {
      ...
      if (!isUserBlockKey(key)) {
        throw new Error(`script-writer-agent: unknown template variable {{${key}}} in system-prompt.md`);
      }
    ...
    if (userTemplateBlocks.length !== USER_BLOCK_KEYS.length) {
      throw new Error(
        `script-writer-agent: expected ${USER_BLOCK_KEYS.length} user-layer blocks in system-prompt.md, found ${userTemplateBlocks.length}`,
      );
    }
 ...
 * (GDE-004 §6.7). Applied uniformly across the entire `storyArchitecture`
 * and `verificationPackage` blocks (README §17).
 */
