 ...
 * truth for the Scene Planner Agent's instructions. This module never
 ...
 *
 * Structurally identical mechanism to `script-reviewer.prompt.ts` (same
 ...
 *
 * Untrusted content handling (GDE-004 §6.7, §13.6, README §14): every
...
export const SCENE_PLANNER_PROMPT_ID = 'prm_scene_planner_agent';

const PROMPT_RELATIVE_SEGMENTS = ['agents', 'agent-07-scene-planner', 'system-prompt.md'] as const;
 ...
 * ever runs from:
 *  - the TypeScript source tree (`src/agents/scene-planner/`, three levels
 *    below the repo root) during development and tests;
 *  - the compiled `dist/src/agents/scene-planner/` output, three levels
 *    below `dist/`, where `npm run build` (`scripts/copy-prompt-asset.js`)
 *    copies the approved file verbatim to
 *    `dist/agents/agent-07-scene-planner/` — the same relative depth.
 *
...
function resolvePromptFile(): string {
  ...
  throw new Error(
    `scene-planner-agent: could not locate the approved prompt asset (system-prompt.md). Checked: ${candidates.join(', ')}. ` +
      'Confirm the production build ran "npm run build" (which copies this asset via scripts/copy-prompt-asset.js).',
  );
}

const USER_BLOCK_KEYS = ['script', 'reviewResult', 'storyArchitecture', 'verificationPackage', 'language'] as const;
...
@Injectable()
export class ScenePlannerPromptService implements OnModuleInit {
   ...
   * Every block (`script`, `reviewResult`, `storyArchitecture`,
  ...
  private loadAndParse(): ParsedPrompt {
    ...
    if (fenced.length !== 2 || systemPrompt === undefined || userTemplate === undefined) {
      throw new Error(
        `scene-planner-agent: expected exactly 2 fenced text blocks in system-prompt.md, found ${fenced.length}`,
      );
    }
    ...
    for (const match of userTemplate.matchAll(BLOCK_PATTERN)) {
      ...
      if (!isUserBlockKey(key)) {
        throw new Error(`scene-planner-agent: unknown template variable {{${key}}} in system-prompt.md`);
      }
    ...
    if (userTemplateBlocks.length !== USER_BLOCK_KEYS.length) {
      throw new Error(
        `scene-planner-agent: expected ${USER_BLOCK_KEYS.length} user-layer blocks in system-prompt.md, found ${userTemplateBlocks.length}`,
      );
    }
 ...
 * `storyArchitecture`, and `verificationPackage` blocks (README §14).
 */
