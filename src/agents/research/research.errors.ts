/**
 * Error construction and classification for the Research Agent runtime.
 *
 * Everything here is pure and deterministic: given a validation finding or a
 * provider failure, it returns a fully-formed `ResearchRuntimeError`. No I/O,
 * no retries, no logging — those belong to the service and to the caller.
 */

import type { ValidationFinding } from '@agents/agent-02-research/interfaces';
import type {
  ErrorCategory,
  ErrorStage,
  PrefixedId,
...
interface BuildErrorInput {
  ...
  readonly details?: ResearchRuntimeError['details'];
  readonly remediation?: string;
  readonly causeChain?: ResearchRuntimeError['causeChain'];
}

/** The one place `occurredAt`, `source`, and `context` are assembled, so every error has an identical shape. */
export function buildRuntimeError(input: BuildErrorInput): ResearchRuntimeError {
 ...
 * `ResearchAgentErrorCode` vocabulary declared in `interfaces.ts`.
 *
 ...
 * (R-IN-001 -> SOURCE_COUNT_BOUNDS_CONTRADICTORY, R-IN-002 -> DUPLICATE_MATERIAL_ID).
 */
export function mapInputFindingToErrorCode(finding: ValidationFinding): ResearchAgentErrorCode {
...
const INPUT_RULE_TO_ERROR_CODE: Readonly<Record<string, ResearchAgentErrorCode>> = {
  'R-IN-001': 'VALIDATION.INPUT.SOURCE_COUNT_BOUNDS_CONTRADICTORY',
  'R-IN-002': 'VALIDATION.INPUT.DUPLICATE_MATERIAL_ID',
  'R-IN-003': 'VALIDATION.INPUT.DUPLICATE_EXISTING_SOURCE_ID',
  'R-IN-004': 'VALIDATION.INPUT.MATERIAL_BOUNDS_EXCEEDED',
};

/**
 * Maps an OUTPUT business-rule violation (R-BUS-*) onto the closed
 * `ResearchAgentErrorCode` vocabulary. Two rules have an unambiguous,
 * consistently-documented dedicated code across every scenario in the
 * approved package's own `test-cases.md`:
 *
 *  - `R-BUS-004` (evidence cites a source that was never declared) is always
 *    `AI_OUTPUT.CONTENT.UNGROUNDED_CLAIM` (cases `05-A`, `08-A`).
 *  - `R-BUS-006` (a source is grounded in neither, or an unresolvable,
 *    provenance reference) is always `AI_OUTPUT.CONTENT.FABRICATED_SOURCE`
 *    (cases `24-A`, `24-B`).
 *
 * Every other output business rule, INCLUDING `R-BUS-009` (conflict
 * `conflictingEvidenceIds`), maps to the generic
 * `AI_OUTPUT.BUSINESS.RULE_VIOLATED`. `R-BUS-009` is a documented exception
 * worth calling out explicitly: `test-cases.md` cites
 * `AI_OUTPUT.BUSINESS.RULE_VIOLATED` for its "fewer than two entries" and
 * "duplicate id" branches (cases `17-B`, `17-D`) but
 * `AI_OUTPUT.CONTENT.UNGROUNDED_CLAIM` for its "unresolved id" branch
 * (case `17-E`) — three different violations reported under one `ruleId`
 * with two different codes in the contract's own documentation. A finding
 * carries only `ruleId`, not which internal branch of that rule produced it,
 * so a deterministic ruleId-keyed map cannot reproduce that finer
 * distinction without inspecting free-text `message` content, which this
 * runtime deliberately does not do (message text is not part of the frozen
 * contract and is not a stable classification key). `R-BUS-009` is therefore
 * folded into the generic code, exactly like every other multi-branch or
 * unremarkable business rule. This is a runtime-mapping simplification, not
 * a contract change — see the implementation report for the full note.
 */
export function mapOutputFindingToErrorCode(finding: ValidationFinding): ResearchAgentErrorCode {
  return OUTPUT_RULE_TO_ERROR_CODE[finding.ruleId] ?? 'AI_OUTPUT.BUSINESS.RULE_VIOLATED';
}

const OUTPUT_RULE_TO_ERROR_CODE: Readonly<Record<string, ResearchAgentErrorCode>> = {
  'R-BUS-004': 'AI_OUTPUT.CONTENT.UNGROUNDED_CLAIM',
  'R-BUS-006': 'AI_OUTPUT.CONTENT.FABRICATED_SOURCE',
};

/** Maps a provider-layer failure kind onto the additive provider error vocabulary (`research.types.ts`). */
export function mapProviderErrorKindToCode(kind: AiProviderErrorKind): ResearchProviderErrorCode {
  switch (kind) {
    case 'TIMEOUT':
 ...
 * filesystem path, or prompt/research-material content into a user-facing
 * field.
 */
