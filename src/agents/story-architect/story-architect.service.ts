...
import type {
  ...
  ExecutionBlock,
  RefusalReasonCode,
  StoryArchitectAgentErrorResponse,
  StoryArchitectAgentRequest,
  StoryArchitectRefusal,
  StoryArchitectResponseMeta,
  StoryArchitecture,
  StandardError,
  ValidationFinding,
...
import {
  ...
  redactKnownSecret,
} from './story-architect.errors';
import { STORY_ARCHITECT_PROMPT_ID, StoryArchitectPromptService } from './story-architect.prompt';
import {
...
import { StoryArchitectExecutionOutcome, StoryArchitectRuntimeError } from './story-architect.types';
...
/**
 * The Story Architecture's JSON Schema definition, extracted from the
 ...
 * ignores it, this service always runs the same `validateStoryArchitecture`
 * pass on the result — see `ai-provider.interface.ts` for why that hint can
 * never replace validation.
 */
const STORY_ARCHITECTURE_JSON_SCHEMA: unknown = {
  root: (storyArchitectureOutputSchema as { $defs: { storyArchitecture: unknown } }).$defs.storyArchitecture,
  defs: (storyArchitectureOutputSchema as { $defs: Record<string, unknown> }).$defs,
};
 ...
 */
const INVOCATION_PARAMETERS = { temperature: 0.7, topP: 1.0, maxOutputTokens: 7000 } as const;
...
/**
 * AGT-04 Story Architect Agent — NestJS runtime.
 *
 ...
 * Agent 01, never calls Agent 03, never calls Agent 05, never orchestrates a
 * workflow, never performs a network request or web search itself, and
 * never persists anything (README §3).
 *
 * `verificationPackage` is the Agent 03 deliverable supplied by the
 * workflow — this service architects a story from it exactly as given; it
 * never re-verifies a claim, never overrides Agent 03's `verificationStatus`
 * or `downstreamSafety`, and never invents a factual claim to fill a gap
 * (README §1, §6).
 */
@Injectable()
export class StoryArchitectService {
  constructor(
    @Inject(STORY_ARCHITECT_REQUEST_VALIDATOR) private readonly requestValidator: ValidateFunction,
    @Inject(STORY_ARCHITECTURE_VALIDATOR) private readonly storyArchitectureValidator: ValidateFunction,
    @Inject(STORY_ARCHITECT_RESPONSE_VALIDATOR) private readonly responseValidator: ValidateFunction,
    @Inject(AI_PROVIDER) private readonly aiProvider: AiProvider,
    @Inject(aiConfig.KEY) private readonly config: ConfigType<typeof aiConfig>,
    private readonly promptService: StoryArchitectPromptService,
  ) {}
   ...
   * `story-architect-agent-output/v1` contract?
   *
   *   AI output -> story architecture schema validation -> business validation
   ...
   */
  async execute(
    request: StoryArchitectAgentRequest,
    options: ExecuteOptions = {},
  ): Promise<StoryArchitectExecutionOutcome> {
   ...
   * itself a conformant `story-architect-agent-output/v1` contract. Always a
  ...
  ): StoryArchitectExecutionOutcome {
  ...
  private async runPipeline(
    ...
    const inputReport = validateStoryArchitectRequest(this.requestValidator, request);
    ...
    //    request problem.
    let rendered: ReturnType<StoryArchitectPromptService['render']>;
    try {
      ...
      return this.failure(
        ...
        [
          buildRuntimeError({
            ...
            userMessage: 'This request could not be prepared for the story architect agent.',
            correlationId,
            runId: request.meta.runId,
            nodeId: request.meta.nodeId,
            attempt,
    ...
    try {
      aiResult = await this.aiProvider.invoke({
        ...
        structuredOutput: { schemaName: 'storyArchitecture', schema: STORY_ARCHITECTURE_JSON_SCHEMA },
      });
    } catch (error) {
      ...
      return this.failure(
        ...
        [
          buildRuntimeError({
            ...
            userMessage:
              'The story architecture could not be completed within the response budget. It may succeed on retry.',
            correlationId,
            runId: request.meta.runId,
            nodeId: request.meta.nodeId,
            attempt,
      ...
      return this.failure(
        ...
        [
          buildRuntimeError({
            ...
            userMessage: 'Story architecture could not be completed right now. It may succeed on retry.',
            correlationId,
            runId: request.meta.runId,
            nodeId: request.meta.nodeId,
            attempt,
    ...
    //    any normal story-architecture processing. Neither outcome below is
    ...
    //       story-architecture parsing/validation or be treated as retryable.
    if (aiResult.finishReason === 'REFUSED') {
      ...
      if (isStoryArchitectRefusal(refusedContent)) {
      ...
      return this.failure(
        ...
        [
          buildRuntimeError({
            ...
            userMessage:
              'The story architecture could not be prepared from the model response. It may succeed on retry.',
            correlationId,
            runId: request.meta.runId,
            nodeId: request.meta.nodeId,
            attempt,
    ...
    //    and stop. Never attempt to salvage a partial story architecture.
    if (isStoryArchitectRefusal(parsed)) {
      return this.refusalFailure(parsed, request, correlationId, attempt, attemptType, startedAt, aiResult);
    }

    // 7. Validate the story architecture structurally, then against business
    //    rules (validator.ts — deterministic, side-effect free, run in that
    //    order). Grounding rules (claim/evidence references, DO_NOT_USE
    //    protection, qualification preservation, duration reconciliation,
    //    readiness consistency) require the request's own data, which is why
    //    it is threaded through here. Invalid model output is never repaired
    //    automatically by this runtime — a business-rule failure is
    //    reported, never silently corrected, and no missing claim is ever
    //    invented to fill a gap (README §6, implementation-checklist.md §4).
    const storyArchitectureReport = validateStoryArchitecture(
      this.storyArchitectureValidator,
      parsed,
      request.data,
    );
    if (storyArchitectureReport.outcome === 'FAILED') {
      ...
      const issues = storyArchitectureReport.findings.slice(0, MAX_REPORTED_ISSUES).map((finding) =>
    ...
    const storyArchitecture = parsed as StoryArchitecture;
    return {
      ok: true,
      response: {
        contractVersion: '1.0',
        contractType: 'RESPONSE',
        schemaVersion: STORY_ARCHITECT_OUTPUT_SCHEMA_VERSION,
        meta: this.responseMeta(request, aiResult),
        status: 'SUCCESS',
        data: storyArchitecture,
    ...
    return this.failure(
      ...
      [
        buildRuntimeError({
          ...
          retryable: false,
          stage: 'OUTPUT_VALIDATION',
          message: `Model declined to produce a story architecture (${refusal.refusal.reasonCode}): ${refusal.refusal.details}`,
          userMessage: isSecurity
            ? 'This request could not be processed because supplied content attempted to alter agent instructions.'
            : 'Story architecture could not be prepared from the supplied inputs.',
  ...
  private inputFindingToRuntimeError(
    ...
    return buildRuntimeError({
      ...
      userMessage: 'Story architecture could not run because the supplied request is invalid.',
      correlationId,
      runId,
      nodeId,
      attempt,
  ...
  private outputFindingToRuntimeError(
    ...
    return buildRuntimeError({
      ...
      stage: 'OUTPUT_VALIDATION',
      message: finding.message,
      userMessage: 'The prepared story architecture did not meet the required quality bar. It may succeed on retry.',
      correlationId,
      runId,
      nodeId,
      attempt,
  ...
  private failure(
    ...
    const response: StoryArchitectAgentErrorResponse = {
      ...
      // interfaces.ts narrows this to AGT-04's own registered vocabulary.
      // Transport/provider-layer issues use the additive codes documented in
      // story-architect.types.ts, which satisfy the wire schema but sit
  ...
  private responseMeta(
    ...
    return {
      ...
      promptVersion: STORY_ARCHITECT_PROMPT_ID,
      ...(aiResult !== undefined
        ? {
...
/**
 * The `schemaVersion` of `story-architect-agent-output/v1` this runtime
 ...
 * §4.2). This mirrors the fix applied to Agent 03's runtime.
 */
const STORY_ARCHITECT_OUTPUT_SCHEMA_VERSION = '1.0.0' as const;

function isStoryArchitectRefusal(value: unknown): value is StoryArchitectRefusal {
