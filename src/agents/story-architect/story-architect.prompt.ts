 ...
 * of truth for the Story Architect Agent's instructions. This module never
 ...
 *
 * Structurally identical mechanism to `fact-verification.prompt.ts`,
 * `research.prompt.ts`, `topic-discovery.prompt.ts`, and `strategy.prompt.ts`
 * (same fenced-block parsing, same delimiter neutralisation, same
 ...
 *
 * Untrusted content handling (GDE-004 §6.7, §13.6, README §15): every
 * rendered value has the `<<<`/`>>>` delimiter sequences neutralised before
 * insertion, applied to the ENTIRE `verificationPackage` block — not a
 * designated subset — since any nested free-text field (a claim's text, a
 * limitation, a note) could carry adversarial text originating from upstream
 * research, even though `verificationPackage` itself is a provenance-TRUSTED,
 * already-validated platform artifact.
 */

export const STORY_ARCHITECT_PROMPT_ID = 'prm_story_architect_agent';

const PROMPT_RELATIVE_SEGMENTS = ['agents', 'agent-04-story-architect', 'system-prompt.md'] as const;
 ...
 * ever runs from:
 *  - the TypeScript source tree (`src/agents/story-architect/`, three levels
 *    below the repo root) during development and tests;
 *  - the compiled `dist/src/agents/story-architect/` output, three levels
 *    below `dist/`, where `npm run build` (`scripts/copy-prompt-asset.js`)
 *    copies the approved file verbatim to
 *    `dist/agents/agent-04-story-architect/` — the same relative depth.
 *
...
function resolvePromptFile(): string {
  ...
  throw new Error(
    `story-architect-agent: could not locate the approved prompt asset (system-prompt.md). Checked: ${candidates.join(', ')}. ` +
...
@Injectable()
export class StoryArchitectPromptService implements OnModuleInit {
   ...
   * Required blocks (`verificationPackage`, `topicOpportunity`,
   * `targetDurationSeconds`, `language`) are always rendered — the input
   * schema guarantees they are present; the optional block
  ...
  private loadAndParse(): ParsedPrompt {
    ...
    if (fenced.length !== 2 || systemPrompt === undefined || userTemplate === undefined) {
      throw new Error(
        `story-architect-agent: expected exactly 2 fenced text blocks in system-prompt.md, found ${fenced.length}`,
      );
    }
    ...
    for (const match of userTemplate.matchAll(BLOCK_PATTERN)) {
      ...
      if (!isUserBlockKey(key)) {
        throw new Error(`story-architect-agent: unknown template variable {{${key}}} in system-prompt.md`);
      }
    ...
    if (userTemplateBlocks.length !== USER_BLOCK_KEYS.length) {
      throw new Error(
        `story-architect-agent: expected ${USER_BLOCK_KEYS.length} user-layer blocks in system-prompt.md, found ${userTemplateBlocks.length}`,
      );
    }
 ...
 * (GDE-004 §6.7). Applied uniformly across the entire `verificationPackage`
 * and `topicOpportunity` blocks (README §15).
 */
