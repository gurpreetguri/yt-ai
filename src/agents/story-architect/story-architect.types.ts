/**
 * Runtime-only types for the Story Architect Agent NestJS module.
 *
 * These are ADDITIVE to the frozen contract in
 * `agents/agent-04-story-architect/interfaces.ts` — nothing here redefines a
 ...
 * runtime (ARC-001 §4.8).
 *
 * Mirrors `src/agents/fact-verification/fact-verification.types.ts`,
 * `src/agents/research/research.types.ts`,
 ...
 * `StoryArchitectAgentErrorCode` — the closed set of codes AGT-04 registers
 * for its OWN generation-domain failures. These follow the same
 * `CATEGORY.SUBJECT.CONDITION` shape and are identical in spelling to the
 * ones AGT-00/01/02/03 already register for the same transport concerns —
 * one error taxonomy applied per agent, not a second one invented for AGT-04.
 */
export type StoryArchitectProviderErrorCode =
   ...
   * of `StoryArchitectService.execute`, before anything is returned). Always
   * a defect in this codebase; non-retryable.
   */
  | 'CONFIGURATION.RUNTIME.RESPONSE_ENVELOPE_INVALID';

export type StoryArchitectRuntimeErrorCode = StoryArchitectAgentErrorCode | StoryArchitectProviderErrorCode;
 ...
 */
export interface StoryArchitectRuntimeError extends Omit<StandardError, 'code'> {
...
export interface StoryArchitectRetryHint {
...
/**
 * What `StoryArchitectService.execute` returns. A discriminated union so
 ...
 * calls Agent 01/03, and never calls Agent 05 (README §3).
 */
export type StoryArchitectExecutionOutcome =
  | { readonly ok: true; readonly response: StoryArchitectAgentResponse & { readonly contractType: 'RESPONSE' } }
  | {
      readonly ok: false;
      readonly response: StoryArchitectAgentResponse & { readonly contractType: 'ERROR' };
      readonly retry: StoryArchitectRetryHint;
    };

/** Everything the service needs to know beyond the request payload. Supplied by the caller (agent runtime), never invented here. */
export interface StoryArchitectExecutionContext {
  readonly request: StoryArchitectAgentRequest;
