...
import {
  STRUCTURAL_RULE_ID,
  structuralValidate,
  validateTopicDiscoveryRequest,
  validateTopicOpportunitySet,
...
import type {
  ...
  RefusalReasonCode,
  StandardError,
  TopicDiscoveryAgentErrorResponse,
  TopicDiscoveryAgentRequest,
  TopicDiscoveryRefusal,
  TopicDiscoveryResponseMeta,
  TopicOpportunitySet,
  ValidationFinding,
...
import {
  ...
  redactKnownSecret,
} from './topic-discovery.errors';
import { TOPIC_DISCOVERY_PROMPT_ID, TopicDiscoveryPromptService } from './topic-discovery.prompt';
import {
...
import { TopicDiscoveryExecutionOutcome, TopicDiscoveryRuntimeError } from './topic-discovery.types';
...
/**
 * The Topic Opportunity Set's JSON Schema definition, extracted from the
 ...
 * ignores it, this service always runs the same `validateTopicOpportunitySet`
 * pass on the result — see `ai-provider.interface.ts` for why that hint can
 * never replace validation.
 */
const TOPIC_SET_JSON_SCHEMA: unknown = {
  root: (topicSetOutputSchema as { $defs: { topicOpportunitySet: unknown } }).$defs.topicOpportunitySet,
  defs: (topicSetOutputSchema as { $defs: Record<string, unknown> }).$defs,
};

/**
 * Sampling parameters fixed by the agent's own prompt contract
 * (system-prompt.md header table: Generator class, temperature 0.7, topP
 * 1.0, max output tokens 6000) — declared here as call-site literals, the
 * same way `strategy.service.ts` fixes AGT-00's own parameters, rather than
 * sourced from the shared `aiConfig`. `aiConfig` remains the source for
 * genuinely cross-agent operational knobs (provider selection, the
 * `timeoutMs` ceiling); it does not own any single agent's declared
 * temperature or token budget, which are part of that agent's frozen prompt
 * contract, not an environment-tunable setting.
 */
const INVOCATION_PARAMETERS = { temperature: 0.7, topP: 1.0, maxOutputTokens: 6000 } as const;
...
> = {
  ...
  INPUT_CONTRADICTORY: { code: 'VALIDATION.INPUT.PILLAR_CONSTRAINTS_CONTRADICTORY', category: 'VALIDATION' },
...
/**
 * AGT-01 Topic Discovery Agent — NestJS runtime.
 *
 ...
 * Agent 00, never calls Agent 02, never orchestrates a workflow, and never
 * persists anything (README §3).
 */
@Injectable()
export class TopicDiscoveryService {
  constructor(
    @Inject(TOPIC_DISCOVERY_REQUEST_VALIDATOR) private readonly requestValidator: ValidateFunction,
    @Inject(TOPIC_DISCOVERY_SET_VALIDATOR) private readonly topicSetValidator: ValidateFunction,
    @Inject(TOPIC_DISCOVERY_RESPONSE_VALIDATOR) private readonly responseValidator: ValidateFunction,
    @Inject(AI_PROVIDER) private readonly aiProvider: AiProvider,
    @Inject(aiConfig.KEY) private readonly config: ConfigType<typeof aiConfig>,
    private readonly promptService: TopicDiscoveryPromptService,
  ) {}
   ...
   * `topic-discovery-agent-output/v1` contract?
   *
   *   AI output -> topic set schema validation -> business validation
   *   -> runtime envelope creation -> FINAL RESPONSE CONTRACT VALIDATION -> return
   *
   * This is deliberately separate from topic-set validation (step 7 in
   * `runPipeline`): that step checks the model's raw output against the
   * `topicOpportunitySet` definition; this step checks the envelope THIS
   ...
   */
  async execute(
    request: TopicDiscoveryAgentRequest,
    options: ExecuteOptions = {},
  ): Promise<TopicDiscoveryExecutionOutcome> {
   ...
   * itself a conformant `topic-discovery-agent-output/v1` contract. Always a
  ...
  ): TopicDiscoveryExecutionOutcome {
  ...
  private async runPipeline(
    ...
    const inputReport = validateTopicDiscoveryRequest(this.requestValidator, request);
    ...
    //    request problem.
    let rendered: ReturnType<TopicDiscoveryPromptService['render']>;
    try {
      ...
      return this.failure(
        ...
        [
          buildRuntimeError({
            ...
            userMessage: 'This request could not be prepared for the topic discovery agent.',
            correlationId,
            runId: request.meta.runId,
            nodeId: request.meta.nodeId,
            attempt,
    ...
    try {
      aiResult = await this.aiProvider.invoke({
        ...
        structuredOutput: { schemaName: 'topicOpportunitySet', schema: TOPIC_SET_JSON_SCHEMA },
      });
    } catch (error) {
      ...
      return this.failure(
        ...
        [
          buildRuntimeError({
            ...
            // never a payload, never a credential, never a path (topic-discovery.errors.ts).
      ...
      return this.failure(
        ...
        [
          buildRuntimeError({
            ...
            userMessage:
              'The topic opportunity set could not be completed within the response budget. It may succeed on retry.',
            correlationId,
            runId: request.meta.runId,
            nodeId: request.meta.nodeId,
            attempt,
      ...
      return this.failure(
        ...
        [
          buildRuntimeError({
            ...
            userMessage: 'Topic discovery could not be completed right now. It may succeed on retry.',
            correlationId,
            runId: request.meta.runId,
            nodeId: request.meta.nodeId,
            attempt,
    ...
    //    any normal topic-set processing. Two distinct outcomes, NEITHER of
    //    which is retryable: a refusal — whether the model's own prompted
    //    refusal or an upstream safety filter — is non-recoverable, because
    //    retrying an identical request is expected to be refused again
    //    (implementation-checklist.md §6, "the agent itself retries nothing";
    //    a refusal MUST NOT consume the retry/regeneration budget):
    //     - the content still carries the structured refusal payload the
    //       prompt's contract requires (system-prompt.md §6) — map it exactly
    //       as an in-band refusal would be mapped (case 8 below), which is
    ...
    //       fall through to topic-set parsing/validation or be treated as
    //       retryable.
    if (aiResult.finishReason === 'REFUSED') {
      ...
      if (isTopicDiscoveryRefusal(refusedContent)) {
      ...
      return this.failure(
        ...
        [
          buildRuntimeError({
            ...
            userMessage:
              'The topic opportunity set could not be prepared from the model response. It may succeed on retry.',
            correlationId,
            runId: request.meta.runId,
            nodeId: request.meta.nodeId,
            attempt,
    ...
    //    and stop. Never attempt to salvage a partial topic set from it.
    if (isTopicDiscoveryRefusal(parsed)) {
      return this.refusalFailure(parsed, request, correlationId, attempt, attemptType, startedAt, aiResult);
    }

    // 7. Validate the topic set structurally, then against business rules
    //    (validator.ts — deterministic, side-effect free, run in that order).
    const topicSetReport = validateTopicOpportunitySet(this.topicSetValidator, parsed, request.data);
    if (topicSetReport.outcome === 'FAILED') {
      ...
      const issues = topicSetReport.findings.slice(0, MAX_REPORTED_ISSUES).map((finding) =>
    ...
    const topicSet = parsed as TopicOpportunitySet;
    return {
      ok: true,
      response: {
        contractVersion: '1.0',
    ...
    return this.failure(
      ...
      [
        buildRuntimeError({
          ...
          retryable: false,
          stage: 'OUTPUT_VALIDATION',
          message: `Model declined to produce a topic opportunity set (${refusal.refusal.reasonCode}): ${refusal.refusal.details}`,
          userMessage: isSecurity
            ? 'This request could not be processed because supplied content attempted to alter agent instructions.'
            : 'Topic discovery could not be prepared from the supplied inputs.',
  ...
  private inputFindingToRuntimeError(
    ...
    return buildRuntimeError({
      ...
      userMessage: 'Topic discovery could not run because the supplied request is invalid.',
      correlationId,
      runId,
      nodeId,
      attempt,
  ...
  private outputFindingToRuntimeError(
    ...
    return buildRuntimeError({
      ...
      stage: 'OUTPUT_VALIDATION',
      message: finding.message,
      userMessage: 'The prepared topic opportunity set did not meet the required quality bar. It may succeed on retry.',
      correlationId,
      runId,
      nodeId,
      attempt,
  ...
  private failure(
    ...
    const response: TopicDiscoveryAgentErrorResponse = {
      ...
      // interfaces.ts narrows this to AGT-01's own registered vocabulary.
      // Transport/provider-layer issues use the additive codes documented in
      // topic-discovery.types.ts, which satisfy the wire schema but sit
      // outside that narrower TS union by design (see topic-discovery.errors.ts
      // for the rationale).
  ...
  private responseMeta(request: TopicDiscoveryAgentRequest, aiResult?: AiInvocationResult): TopicDiscoveryResponseMeta {
    return {
      ...
      promptVersion: TOPIC_DISCOVERY_PROMPT_ID,
      ...(aiResult !== undefined
        ? {
...
function isTopicDiscoveryRefusal(value: unknown): value is TopicDiscoveryRefusal {
